...
- HTML：能写 `<div>`、`<input>`、`<button>`、`<ul><li>`
- CSS：知道 class、id、基础选择器
- JavaScript：变量 `let/const`、箭头函数、数组方法（`map/filter/find/push`）、对象、`Promise/async-await`、模块 `import/export`
- 命令行：`npm install`、`npm run xxx` 能跑通
- 已安装 Node.js（建议 18+）
